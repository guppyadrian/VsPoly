...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		//guppy- wow i can code i made second dialogue box i am so cool
		var hasDialog = false;
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'bitwit' | 'polygonal' | 'hexadec' | 'subaru':
				...
				box.animation.addByPrefix('normalOpen', 'speech bubble loud open', 5, false);
				box.animation.addByPrefix('normal', 'AHH speech bubble', 5);
				boxBf = new FlxSprite(-20, 375);
				boxBf.frames = Paths.getSparrowAtlas('poly_bubble_talking');
				boxBf.animation.addByPrefix('normalOpen', 'Speech Bubble Normal Open', 5, false);
				boxBf.animation.addByPrefix('normal', 'speech bubble normal', 5);
				boxBf.visible = false;
			case 'roses':
				box = new FlxSprite(-20, 45);
				hasDialog = true;
		...
		switch (PlayState.SONG.song.toLowerCase()) {
			...
			default:
				...
				portraitRight.frames = Paths.getSparrowAtlas('portraits/bfNormalPortrait');
				portraitRight.animation.addByPrefix('enter', 'Boyfriend portrait enter', 24, false);
				portraitRight.updateHitbox();
				portraitRight.scrollFactor.set();
				add(portraitRight);
		...
		portraitMiddle = new FlxSprite(-25, 10);
		portraitMiddle.frames = Paths.getSparrowAtlas('portraits/gfPortrait');
	...
	override function update(elapsed:Float)
	{
		...
		if (box.animation.curAnim != null)
		{
			if (box.animation.curAnim.name == 'normalOpen' && box.animation.curAnim.finished)
			{
				...
				if (twoBoxes) boxBf.animation.play('normal'); //the two box thing barely works
				dialogueOpened = true;
			}
		}
