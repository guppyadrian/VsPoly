...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		DiscordClient.changePresence("Popping off in " + detailsText + " " + SONG.song + " (" + storyDifficultyText + ") " + generateRanking(), "\nAcc: " + truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
		#end
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 3 ? "Flushed" : storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
	...
	function polyIntro(?dialogueBox:DialogueBox, end:Bool = false):Void
	{
		if (dialogueBox != null)
		{
			inCutscene = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong == 'polygonal')
		{
			switch (curBeat)
			{
				case 385:
					boyfriend.playAnim('shocked');
					gf.playAnim('scared');
				case 386:
					boyfriend.playAnim('shockHold');
				case 416:
		...
		if (health <= 0)
		{
			...
			DiscordClient.changePresence("THANOS SNAPPED -- " + SONG.song + " (" + storyDifficultyText + ") " + generateRanking(),"\nAcc: " + truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
			#end
	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			...
			{
				if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && boyfriend.animation.curAnim.name != 'shocked' && boyfriend.animation.curAnim.name != 'hey' && boyfriend.animation.curAnim.name != 'shockHold')
				{
					boyfriend.playAnim('idle');
				}
			}
	...
	*/
	...
	override function beatHit()
	{
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && boyfriend.animation.curAnim.name != 'shockHold' && boyfriend.animation.curAnim.name != 'shocked')
		{
			boyfriend.playAnim('idle');
		}
